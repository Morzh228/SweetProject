...
def application():
    ...
    opt.pack(side="top")
    print(variable.get())
    if variable.get() == f'{list_as_name[0]}':
        # Выпадающий список метрик
        list_metric = postgres("SELECT metric_name FROM metric WHERE as_name_id = 5")
        variable = StringVar(root)
        variable.set(list_metric[0])

        opt = OptionMenu(root, variable, *list_metric)
        opt.config(width=90, font=('Helvetica', 12))
        opt.pack(side="top")
